...
def main():
    ...
    if holders:  # реальный открытый дескриптор — жёсткий блок
        ...
        # lsof ненадёжен (CC не держит файл постоянно). Если файл менялся только
        # что — мягко предупреждаем, но НЕ блокируем (мог быть момент закрытия).
        try:
            age = time.time() - os.path.getmtime(path)
        except OSError:
            age = 1e9
        if age < 60:
            print(f"ℹ файл менялся {int(age)} сек назад. Если сессия ещё открыта — "
                  "рез не подействует; убедись, что закрыл её.\n")
